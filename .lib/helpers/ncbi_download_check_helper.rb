...
class NcbiDownloadCheckHelper
    ...
    COX1_RELEASE247_GBINV = '.config/cox1_release247_gbinv.txt'
    ...
    def self.ask_user_about_download_dirs(params, only_successful = true)
        ...
        dirs.each { |dir| releases.push(create_release_info_struct(dir, params, division_codes)) }
    ...
    def self.create_release_info_struct(dir, params, division_codes = nil)
        ## TODO: make sure that descend! in writing download info files
        ## will not brake since its not a relative path from the taxalogue repo

        division_codes      = NcbiDivision.codes_for_taxon(params) unless division_codes
