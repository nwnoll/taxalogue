...
class BoldJob
  ...
  def run4
    ...
    num_of_ranks.times do |i|
      ...
      Parallel.map(root_node.entries, in_threads: 5) do |node|
        ...
        puts "... downloading #{node.name} with worker: #{Parallel.worker_number}"
  ...
  def run3
    ...
    num_of_ranks.times do |i|
      root_node.print_tree(level = root_node.node_depth, max_depth = nil, block = lambda { |node, prefix| puts "#{prefix} #{node.name} - #{node.content.last}" })
      i == 0 ? processes_num = 1 : processes_num = 3
      # p nodes
      Parallel.map(root_node.entries, in_threads: 5) do |node|
        next unless node.content.last == 'pending'
  ...
  def run2
    ...
    num_of_ranks.times do |i|
      root_node.print_tree(level = root_node.node_depth, max_depth = nil, block = lambda { |node, prefix| puts "#{prefix} #{node.name} - #{node.content.last}" })
      i == 0 ? processes_num = 1 : processes_num = 3
      nodes = [root_node, root_node.children].flatten
      # p nodes
      Parallel.map(nodes, in_threads: 3) do |node|
        next unless node.content.last == 'pending'
