    ...
    file_names.each do |file|
      ...
      Zlib::GzipReader.open(file) do |gz_file|
        ...
        ff.each_entry do |gb|
          ...
          # # country = nil
          # # features_of_source.each { |f| f.qualifiers.each { |q| q.qualifier == 'country' ? country = q.value : nil } }

          # country = _get_country_value(source_features)

          # # lat_lon = nil
          # # features_of_source.each { |f| f.qualifiers.each { |q| q.qualifier == 'lat_lon' ? lat_lon = q.value : nil } }
          # lat_lon =  _get_lat_lon_value(source_features)
          # # country_value = nil
          # # features_of_source.each { |f| _get_country_value(f.qualifiers) }
          # p '------'
          # p country
          # p lat_lon
          # p '------'
